...
class Zombie:
    ...
    # already occupied by another zombie 2) are the new coordinates outside of the arena
    def walk(self,newx, newz):
        cube = mc.getBlockWithData(newx, Y, newz)
        if cube.id == block.WOOL.id:                                    # can't walk if there's another zombie already there
            return
